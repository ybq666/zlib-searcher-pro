...
### 5. 🔑 多模式账号登录与下载配额绑定
为了解决部分镜像站 Cookie 隔离或跨域读取问题，插件提供了三种极简的登录认证方式：
- **方式一：账号密码直接登录（最推荐）**：
  - 打开设置中心，在“认证与凭据管理”中直接输入您的 **Z-Library 账号（邮箱）和密码**；
  - 点击“登录并绑定凭据”，插件将通过官方 `/eapi/user/login` 接口直连登录，自动获取 `remix_userid` 与 `remix_userkey` 并同步激活。
- **方式二：从浏览器已登录页面一键嗅探 Cookie**：
  - 如果您在弹窗中点击“网页登录”或在任意官方镜像站、SingleLogin 网页完成了登录或验证；
  - 返回插件弹窗点击 **“提取凭据”**（或在设置页点击 **“🔍 一键扫描并提取 Cookie”**），插件会自动跨域检索全局已存 Cookie 并立即同步到当前使用的节点。
- **方式三：手动输入凭据（高级备选）**：
  - 支持直接在设置中心手动保存 `remix_userid` 与 `remix_userkey`，多设备备份更安心。
